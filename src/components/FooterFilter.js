...
export default function FooterFilter({ tasks, setTasks, setFilteredTasks, filteredTasks }) {
  ...
  return (
    <ul className="filters">
      ...
      <li>
        <button onClick={handleShowActive} className={selectedButton === 'Active' ? 'selected' : ''}>
          Active
        </button>
      </li>
      <li>
        <button onClick={handleShowCompleted} className={selectedButton === 'Completed' ? 'selected' : ''}>
          Completed
        </button>
      </li>
    </ul>
